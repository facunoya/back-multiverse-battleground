...
const apiControllers = {
  getAllUsers: async (req, res) => {
    try {
      ...
      const users = await db.Users.findAll({
        include: [
          {
            association: "userfighters",
            where: whereCondition,
            include: [{ model: db.Fighters, as: "fighters" }],
          },
          {
            association: "userobjects",
            where: whereCondition,
            include: [
              {
                model: db.Objects,
  ...
  getAllObjects: async (req, res) => {
    const object_id = req.params.object_id ? req.params.object_id : null
    // Construir la condición de búsqueda
    const whereCondition = object_id ? { object_id: object_id } : {}
    await db.Objects.findAll({ where: whereCondition }).then((objects) => {
      return res.send(objects)
    })
  },
  getObjectActions: async (req, res) => {
    const object_id = req.params.object_id ? req.params.object_id : null
    // Construir la condición de búsqueda
    const whereCondition = object_id ? { object_id: object_id } : {}
    await db.ObjectActions.findAll({ where: whereCondition }).then(
      (actions) => {
  ...
  getAllUserObjects: async (req, res) => {
    ...
    await db.UserObjects.findAll({
      include: [
        { association: "objects", include: [{ association: "actionobjects" }] },
      ],
  ...
  upadateUserObjects: async (req, res) => {
    ...
    objects.forEach(async (object) => {
      ...
      if (quantity > 0 && updateObject.object_id !== 7) {
        ...
        await db.UserObjects.update(
          { ...updateObject, quantity: quantity },
          { where: { user_object_id } }
        ) //esperar a que este resuelto
      } else {
  ...
  getAllFighterLevels: async (req, res) => {
    ...
    await db.FighterLevels.findAll({
      where: whereCondition,
      include: [{ association: "fighters" }],
    }).then((fighterLevel) => {
  ...
  getAllMoves: async (req, res) => {
    await db.Moves.findAll({
      include: [{ association: "fighters" }, { association: "actionmoves" }],
    }).then((moves) => {
      return res.send(moves)
    })
  },
  getMoveActions: async (req, res) => {
    ...
    await db.MoveActions.findAll({ where: { move_id } }).then((actions) => {
      return res.send(actions)
    })
  },
  getAllFighterMoves: async (req, res) => {
    await db.UserFighterMoves.findAll({
      include: [{ association: "userfighters" }, { association: "moves" }],
    }).then((moves) => {
      return res.send(moves)
    })
  },
  ...
  updateFighter: async (req, res) => {
    ...
    moves.forEach((move) => {
      ...
      moveLevels.forEach((moveLevel) => {
        if (
          moveLevel.move_id === move.move_id &&
          moveLevel.min_xp < move.current_xp
        ) {
    ...
    const fighterMoves = await db.Moves.findAll({
      where: {
        fighter_id: userFighter.fighter_id,
        min_level: {
  ...
  updateUserConfig: async (req, res) => {
    ...
    await db.Users.update(
      { ...updateUser, bg_volume: bg, sfx_volume: sfx, sound_volume: sound },
      { where: { user_id } }
    )
    res.send("ok")
  },
